...
public class BidService {
    ...
    public void placeBid(PlaceBidRequest placeBidRequest, String bidderId, String bidderUsername) throws ServiceException, Exception {
        try {
            ...
            logger.log(Level.SEVERE, "Unexpected error during manual bid placement for bidderId: " + bidderId, exception);
            throw exception;
        }
    }
    ...
    private void validatePlaceBidRequest(PlaceBidRequest placeBidRequest, String bidderId) throws ServiceException, Exception {
        try {
            ...
            if (bidderId == null || bidderId.isBlank()) {
                throw new ServiceException(ErrorCode.MISSING_BIDDER_ID, "The bidder identifier is missing from the current session.");
            }
            if (!BidValidator.isPositiveBid(placeBidRequest.getBidAmount())) {
                throw new ServiceException(ErrorCode.INVALID_BID_AMOUNT, "The bid amount must be a positive value greater than zero.");
            }
        } catch (ServiceException serviceException) {
            throw serviceException;
        } catch (Exception exception) {
            logger.log(Level.SEVERE, "Unexpected error during bid request validation", exception);
            throw exception;
        }
    }
     ...
     * @throws ServiceException if the auction is not found or has already concluded.
     */
    private Auction retrieveAndValidateAuction(String auctionId) throws ServiceException, Exception {
        try {
            ...
            logger.log(Level.WARNING, "Service exception during auction retrieval for auctionId: " + auctionId, serviceException);
            throw serviceException;
        } catch (Exception exception) {
            logger.log(Level.SEVERE, "Unexpected error during auction retrieval and validation for auctionId: " + auctionId, exception);
            throw exception;
        }
    }
}
